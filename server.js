    ...
    try {
        ...
        if (sessionPayload?.email) {
            mapped = await withDb(async (client) => {
                const r = await client.query('SELECT email, name, role, manager_email, is_active FROM users WHERE email = $1 LIMIT 1', [normalizeEmail(sessionPayload.email)]);
            ...
            mapped = await withDb(async (client) => getOrProvisionUser(client, payload));
        }
