...
const processIncomingMessage = async (from, name, msgBody, msgType = 'text', timestamp = Date.now()) => {
      ...
      try {
          ...
          // Check if we need to auto-activate bot (For New Drivers)
          if (!driver) {
            isNewDriver = true;
            // Bot is active initially for BOT_ONLY and HYBRID
          ...
          // STRATEGY: BOT ONLY or HYBRID
          else if (botSettings.isEnabled) {
              
              // RESTART LOGIC FOR BOT_ONLY
              // If strategy is BOT_ONLY and the bot was previously finished (inactive), 
              // we restart it from the beginning upon receiving a new message.
              if (routingStrategy === 'BOT_ONLY' && !driver.is_bot_active && !isNewDriver) {
                  ...
                  if (firstStepId) {
                      ...
                      // Update local object to reflect restart
                      driver.is_bot_active = true;
                      driver.current_bot_step_id = firstStepId;
                      // Treat this message as the "Trigger" so we send the Welcome message immediately
                      isNewDriver = true; 
                  }
              }
              ...
              if (driver.is_bot_active && driver.current_bot_step_id) {
                  ...
                  if (currentStep) {
                      // If NOT new driver/restart, this message is a RESPONSE to a previous step
                      if (!isNewDriver) {
                          // Save Data
                          ...
                          if (nextId === 'AI_HANDOFF' || nextId === 'END') {
                               ...
                               if (nextId === 'AI_HANDOFF') {
                                   if (routingStrategy === 'HYBRID_BOT_FIRST') {
                                       ...
                                       // In BOT_ONLY, AI Handoff acts as an endpoint
                                       replyText = "Thank you. We will contact you soon.";
                                   }
                               } else {
                                   // END
                               ...
                               if (nextStep) {
                                   ...
                                   // Configuration Error: Step missing. Fallback to AI in Hybrid, or Generic in Bot Only.
                                   if (routingStrategy === 'HYBRID_BOT_FIRST') shouldCallAI = true;
                                   else replyText = "Configuration Error. Please contact support.";
                               }
                          }
                      } else {
                           // Is New Driver / Restart: Trigger FIRST step immediately
                  ...
                  // This branch is hit for HYBRID when bot flow is done.
                  if (routingStrategy === 'HYBRID_BOT_FIRST') {
                      shouldCallAI = true;
                  }
              }
          ...
          await client.query('COMMIT'); 
          // Commit DB changes before external API calls to avoid locking if API is slow
