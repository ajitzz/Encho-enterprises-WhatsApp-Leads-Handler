...
const logSystemMessage = async (driverId, text, type = 'text', headerImg = null, footer = null, btns = null) => {
    const msgId = `sys_${Date.now()}_${Math.random()}`;
    await queryWithRetry(
        `INSERT INTO messages (id, driver_id, sender, text, timestamp, type, header_image_url, footer_text, buttons) VALUES ($1, $2, 'system', $3, $4, $5, $6, $7, $8)`, 
        [msgId, driverId, text, Date.now(), type, headerImg, footer, btns ? JSON.stringify(btns) : null]
    );
...
router.post('/messages/send', async (req, res) => {
    try {
        ...
        if (sent) {
            ...
            await queryWithRetry(`INSERT INTO messages (id, driver_id, sender, text, timestamp, type, image_url, header_image_url, footer_text, buttons) VALUES ($1, $2, 'agent', $3, $4, $5, $6, $7, $8, $9)`, 
                [`ag_${Date.now()}`, driverId, text, Date.now(), type, mediaUrl, headerImageUrl, footerText, buttons ? JSON.stringify(buttons) : null]
            );
