...
const upsertCandidateFromInbound = async ({ client, phoneNumber, name, lastMessage, nowMs }) => {
...
const findInboundMessageByWhatsappId = async ({ client, whatsappMessageId }) => {
  const result = await client.query('SELECT id FROM candidate_messages WHERE whatsapp_message_id = $1', [whatsappMessageId]);
  return result.rows[0] || null;
};

const insertInboundMessage = async ({ client, candidateId, text, type, whatsappMessageId }) => {
  await client.query(
