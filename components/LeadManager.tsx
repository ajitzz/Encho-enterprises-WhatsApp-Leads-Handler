                      ...
                      <div className="space-y-4">
                          ...
                          <div>
                              <label className="text-xs font-bold text-gray-500 uppercase mb-2 flex items-center gap-2">
                                  <LayoutTemplate size={14} /> Meta Template (Optional)
                              </label>
                              <input 
                                  value={templateName}
                                  onChange={(e) => setTemplateName(e.target.value)}
                                  className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="e.g. welcome_message_v2"
                              />
                              <p className="text-[10px] text-gray-400 mt-1">If set, text/media below will map to template variables.</p>
                          </div>

                          <div>
                              <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Message / Body Variable</label>
                              <textarea value={bulkMessage} onChange={(e) => setBulkMessage(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl resize-none outline-none focus:ring-2 focus:ring-blue-500 min-h-[120px] text-sm" placeholder="Type message or {{1}} value..." />
                          </div>
                          ...
                          {/* Buttons (Only shown if NOT using template, as Templates define their own buttons usually) */}
                          {!templateName && (
                              <div>
                                  <div className="flex items-center justify-between mb-2">
