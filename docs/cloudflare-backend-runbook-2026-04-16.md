...
- `wrangler.jsonc` defines only assets when `main` is missing, which causes static-assets-only behavior.
...
- `wrangler.jsonc` is now the single source of truth and defines `main = "src/worker.ts"`, so Cloudflare no longer classifies deployment as static-assets-only.
